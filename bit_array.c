...
char bit_array_load_mm(BIT_ARRAY * bitarr, unsigned char * mm, uint64_t offset){
    // Read in number of bits, return 0 if we can't read in
    bit_index_t num_bits = *((uint64_t *)(mm+offset));
    ...
    memcpy(bitarr->words, (uint64_t *)(mm+sizeof(uint64_t)+offset), num_of_bytes);
